...
def get_tile_map_position(x, y, first_tile_x):
    continuous_x = np.abs(x - first_tile_x)
    ret_x = 0
    if continuous_x != 0:
        ret_x = int((continuous_x - (x % TILE_WIDTH))/TILE_WIDTH)

    ret_y = int(y / TILE_HEIGHT)
...
#
# def create_sensors(player, window):
#     y_unit = HEIGHT_SPACES_SIZE + SENSOR_HEIGHT
#     x_unit = WIDTH_SPACES_SIZE + SENSOR_WIDTH
#
#     player_x = player.rect.x - SENSOR_PLAYER_OFFSET_X
#     player_y = player.rect.y - SENSOR_PLAYER_OFFSET_Y
#
...
#         current_x = player_x - x_unit * SENSOR_LEFT_LINES + WIDTH_SPACES_SIZE
#         line.append(Sensor(window, current_x, current_y, SENSOR_WIDTH, SENSOR_HEIGHT))
#         for col_index in range(0, int((SENSOR_MATRIX_COLUMNS-1)/2)):
#             current_x += 2 * x_unit
#             line.append(Sensor(window, current_x, current_y, SENSOR_WIDTH, SENSOR_HEIGHT))
#         sensors.append(line)
#
...
#         current_x = player_x - x_unit * SENSOR_LEFT_LINES + WIDTH_SPACES_SIZE
#         line.append(Sensor(window, current_x, current_y, SENSOR_WIDTH, SENSOR_HEIGHT))
#         for x in range(0, int((SENSOR_LEFT_LINES-1)/2)):
#             current_x += 2 * x_unit
#             line.append(Sensor(window, current_x, current_y, SENSOR_WIDTH, SENSOR_HEIGHT))
#
#         # dreapta
#         current_x = player_x + player.rect.width + x_unit #+ WIDTH_SPACES_SIZE
#         line.append(Sensor(window, current_x, current_y, SENSOR_WIDTH, SENSOR_HEIGHT))
#
#         for x in range(0, int((SENSOR_RIGHT_LINES-1)/2)):
#             current_x += 2* x_unit
#             line.append(Sensor(window, current_x, current_y, SENSOR_WIDTH, SENSOR_HEIGHT))
#
#         sensors.append(line)
